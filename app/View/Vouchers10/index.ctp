...
function showData(){
    $('#lodding_image').show();
    var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers10','action'=>'indexAjax'))?>";
...
function deleteVoucher10(voucher10Id){
    if(voucher10Id != ''){
        if(confirm('Are you sure to delete?')){
            var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers10','action'=>'voucher10Delete'))?>";
            $.post(url,{id:voucher10Id},function(res){ 
                if(res.trim() == 'SUCC'){
                    $('#tr'+voucher10Id).remove();
                }
            }); 
        }
    }
}
function approveVoucher10(voucher10Id){
    if(voucher10Id != ''){
        if(confirm('Are you sure to approve?')){
            var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers10','action'=>'voucher10Approve'))?>";
            $.post(url,{id:voucher10Id},function(res){ 
                if(res.trim() == 'SUCC'){
                    $('#td'+voucher10Id).html('<span style="color:green;font-weight:bold;">Approved Successfully</span>');
                }
            }); 
        }
    }    
}
function rejectVoucher10(voucher10Id){
    if(voucher10Id != ''){
        var remarks=prompt("Enter Remarks");
        if(remarks){
            if(confirm('Are you sure to reject?')){
                var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers10','action'=>'voucher10Reject'))?>";
                $.post(url,{id:voucher10Id,'remarks':remarks},function(res){ 
                    if(res.trim() == 'SUCC'){
                        $('#td'+voucher10Id).html('<span style="color:red;font-weight:bold;">Rejected Successfully</span>');
                    }
                }); 
            }
        }
