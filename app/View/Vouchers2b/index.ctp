...
function showData(){
    $('#lodding_image').show();
    var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers2b','action'=>'indexAjax'))?>";
...
function deleteVoucher2b(voucher2bId){
    if(voucher2bId != ''){
        if(confirm('Are you sure to delete?')){
            var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers2b','action'=>'voucher2bDelete'))?>";
            $.post(url,{id:voucher2bId},function(res){ 
                if(res.trim() == 'SUCC'){
                    $('#tr'+voucher2bId).remove();
                }
            }); 
        }
    }
}
function approveVoucher2b(voucher2bId){
    if(voucher2bId != ''){
        if(confirm('Are you sure to approve?')){
            var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers2b','action'=>'voucher2bApprove'))?>";
            $.post(url,{id:voucher2bId},function(res){ 
                if(res.trim() == 'SUCC'){
                    $('#td'+voucher2bId).html('<span style="color:green;font-weight:bold;">Approved Successfully</span>');
                }
            }); 
        }
    }    
}
function rejectVoucher2b(voucher2bId){
    if(voucher2bId != ''){
        var remarks=prompt("Enter Remarks");
        if(remarks){        
            if(confirm('Are you sure to reject?')){
                var url = "<?php echo $this->Html->url(array('controller'=>'Vouchers2b','action'=>'voucher2bReject'))?>";
                $.post(url,{id:voucher2bId,'remarks':remarks},function(res){ 
                    if(res.trim() == 'SUCC'){
                        $('#td'+voucher2bId).html('<span style="color:red;font-weight:bold;">Rejected Successfully</span>');
                    }
                }); 
            }
        }
